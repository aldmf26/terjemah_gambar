    ...
    <div class="card-body">
        ...
        <form action="{{ route('quizzes.update', $quiz->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="question" class="form-control" value="{{ old('question', $quiz->question) }}" required>
            </div>

            <div class="mb-3">
                <label class="form-label">Tipe Quiz</label>
                <select name="type" class="form-control" id="quizType" required>
                    <option value="multiple_choice" {{ $quiz->type == 'multiple_choice' ? 'selected' : '' }}>Pilihan Ganda</option>
                    <option value="true_false" {{ $quiz->type == 'true_false' ? 'selected' : '' }}>Benar / Salah</option>
            ...
            {{-- Pilihan Jawaban --}}
            <div class="mb-3" id="optionsWrapper" style="{{ $quiz->type != 'multiple_choice' ? 'display:none' : '' }}">
                <label class="form-label">Pilihan Jawaban</label>
                <div id="optionList">
                    @foreach($quiz->options as $i => $option)
                        <div class="d-flex mb-2 option-item">
                            <input type="text" name="options[{{ $i }}][text]" class="form-control me-2" value="{{ $option->option_text }}" required>
                            <div class="form-check me-2">
                                <input class="form-check-input" type="checkbox" name="options[{{ $i }}][is_correct]" {{ $option->is_correct ? 'checked' : '' }}>
